...
| **Blender** | ✅ (headless) | 4.2.3 — **automação headless ativa**: exportação multi-formato + limpeza/decimate + **remesh watertight com re-bake (EMIT)** + **ajuste auto de textura (`texfix`)** (`services/blender-service/`). Retopo quad *puro* (QuadriFlow) segue instável → voxel remesh é o caminho confiável |
...
| Ajuste automático de texturas | ✅ | **Op `texfix`** (Blender headless): ajuste automático do albedo **preservando a matiz** — níveis por razão de luminância (contraste/brilho) + saturação suave; ignora padding do atlas. (White-balance gray-world foi descartado: estragava cores fortes legítimas.) Exposto na tela Exportar e auto-aplicado no pacote 1-clique (`texture_fix`). Validado E2E (cogumelo: realce sem virar ciano) |
...
| Tela Exportar (na UI) | ✅ | **Funcional**: otimizar (cleanup/decimate/remesh) + ajuste de textura + exportar GLB/GLTF/OBJ/FBX/STL/USDZ/PLY via Blender headless. Validada E2E |
| Tela Workflow (na UI) | 🚧 | Placeholder honestamente marcado "em desenvolvimento" (editor de nós é futuro) — NÃO finge funcionar |
