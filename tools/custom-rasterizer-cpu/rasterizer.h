...
// Build CPU-only (MeshForge): cabecalho adaptado para compilar sem o CUDA
// Toolkit (sem nvcc). Diferencas vs original:
//   - removido <ATen/cuda/CUDAContext.h> (so o caminho GPU precisa);
//   - __host__/__device__ neutralizados (sao palavras-chave do nvcc).
// O dispatcher rasterize_image escolhe CPU vs GPU pelo device do tensor;
// aqui o simbolo rasterize_image_gpu vem de um stub (lanca erro se chamado).
